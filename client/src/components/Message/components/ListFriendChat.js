...
const ListFriendChat = ({ user }) => {
  ...
  useEffect(() => {
    if (rooms) {
      // Extract existing room IDs
      const existingRoomIds = rooms.map((room) => room.convId);
      setExistingRooms(existingRoomIds);
    }
  }, [rooms]);
    ...
    <Box flex={1} p={1} sx={{ background: "#535151", height: 1000 }}>
      <Stack direction="column" justifyContent="space-between">
        ...
        >
          <GroupOutlinedIcon
            fontSize="large"
            sx={{ marginLeft: 4, fontSize: 40 }}
          />
          <Typography variant="body1" color="#535151">
            Friends
          </Typography>
        </Box>
        <Stack
        ...
        {rooms.map(
          (room) =>
            !existingRooms.includes(room.convId) && (
