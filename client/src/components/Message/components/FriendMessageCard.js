...
const FriendMessageCard = ({user,room, handleRoomClick}) => {
  ...
  return (
    <>
        <Box display='flex' flexDirection='row' onClick={() => handleRoomClick(room.convId)} alignItems='center' gap='20px' margin='15px 20px 5px' sx={{ background: 'white', borderRadius: 2, width: '90%', height: '80px', boxShadow: 'rgba(50,20,250, 0.3) 0px 7px 29px 0px', '&:hover': { background: 'whitesmoke' }, cursor: 'pointer' }}>
            <Avatar alt="avatar_img" src={user.user_id === room.sendUser.user_id ? room.receiveUser.avatar_url : room.sendUser.avatar_url} sx={{ width: '60px', height: '60px', objectFit: 'cover', marginLeft: 2 }} />
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'start', alignItems: 'start' }}>
                <Typography>{user.user_id === room.sendUser.user_id ? room.receiveUser.userName : room.sendUser.userName}</Typography>
                <div style={{ display: 'flex', flexDirection: 'row', gap: 5, alignItems: 'center' }}>
                    <img alt='icon' src={Online} width='15px' height='15px' />
                    <Typography variant='body2'>Online</Typography>
                </div>
